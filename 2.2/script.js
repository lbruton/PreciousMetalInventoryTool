...
const updateSummary = () => {
  ...
  elements.totals.silver.avgCollectablePrice.innerHTML = formatDollar(silver.avgCollectablePrice);
  elements.totals.silver.avgNonCollectablePrice.innerHTML = formatDollar(silver.avgNonCollectablePrice);
  ...
  // Add the new collectable/non-collectable averages
  elements.totals.gold.avgCollectablePrice.innerHTML = formatDollar(gold.avgCollectablePrice);
  elements.totals.gold.avgNonCollectablePrice.innerHTML = formatDollar(gold.avgNonCollectablePrice);

  // CORRECTED: Proper weighted averaging for "All" totals
  // Calculate total collectable and non-collectable weights across both metals
  const totalCollectableWeight = silver.collectableWeight + gold.collectableWeight;
  const totalNonCollectableWeight = silver.nonCollectableWeight + gold.nonCollectableWeight;

  // Calculate weighted averages for collectable and non-collectable prices
  const avgCollectablePriceAll = totalCollectableWeight > 0 ? 
    (silver.collectableValue + gold.collectableValue) / totalCollectableWeight : 0;
  const avgNonCollectablePriceAll = totalNonCollectableWeight > 0 ? 
    (silver.nonCollectableValue + gold.nonCollectableValue) / totalNonCollectableWeight : 0;
  ...
  // CORRECTED: Use the properly calculated weighted averages
...
const toggleCollectable = (idx, checkbox) => {
  ...
  if (wasCollectable && !isCollectable) {
    ...
    if (spotPrice <= 0) {
      spotPrice = item.metal === 'Silver' ? spotSilver : spotGold;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Add the new collectable/non-collectable average elements
  elements.totals.silver.avgCollectablePrice = document.getElementById('avgCollectablePriceSilver');
  elements.totals.silver.avgNonCollectablePrice = document.getElementById('avgNonCollectablePriceSilver');
  ...
  // Add the new collectable/non-collectable average elements
  elements.totals.gold.avgCollectablePrice = document.getElementById('avgCollectablePriceGold');
  elements.totals.gold.avgNonCollectablePrice = document.getElementById('avgNonCollectablePriceGold');
  ...
  // Add the new collectable/non-collectable average elements
  elements.totals.all.avgCollectablePrice = document.getElementById('avgCollectablePriceAll');
  elements.totals.all.avgNonCollectablePrice = document.getElementById('avgNonCollectablePriceAll');

...
const clearApiKey = (provider) => {
  ...
  if (provider === "CUSTOM") {
    config.custom = { baseUrl: "", endpoint: "", metalFormat: "word" };
    const baseUrlInput = document.getElementById("customBaseUrl");
    const endpointInput = document.getElementById("customEndpoint");
    const formatSelect = document.getElementById("customMetalFormat");
    const display = document.getElementById("customBaseUrlDisplay");
    if (baseUrlInput) baseUrlInput.value = "";
    if (endpointInput) endpointInput.value = "";
    if (formatSelect) formatSelect.value = "word";
    if (display) display.textContent = "User-defined";
  }
  saveApiConfig(config);
...
const fetchSpotPricesFromApi = async (provider, apiKey) => {
  ...
  if (provider === "CUSTOM") {
    const { baseUrl, endpoint, metalFormat } = apiConfig.custom || {};
    const codeMap = {
      silver: metalFormat === "symbol" ? "XAG" : "silver",
      gold: metalFormat === "symbol" ? "XAU" : "gold",
      platinum: metalFormat === "symbol" ? "XPT" : "platinum",
      palladium: metalFormat === "symbol" ? "XPD" : "palladium",
    };
    for (const metal of Object.keys(codeMap)) {
      try {
        const url = `${baseUrl}${endpoint
          .replace("{API_KEY}", apiKey)
          .replace("{METAL}", codeMap[metal])}`;
...
const syncSpotPricesFromApi = async (
  ...
  if (apiConfig.provider === "CUSTOM") {
    const { baseUrl, endpoint } = apiConfig.custom || {};
    if (!baseUrl || !endpoint) {
      alert("Custom provider configuration incomplete.");
      return false;
    }
  }
...
const testApiConnection = async (provider, apiKey) => {
  try {
    ...
    if (provider === "CUSTOM") {
      const { baseUrl, endpoint, metalFormat } = apiConfig.custom || {};
      const metalParam = metalFormat === "symbol" ? "XAG" : "silver";
...
const handleProviderSync = async (provider) => {
  ...
  if (provider === "CUSTOM") {
    ...
    const metalFormat =
      document.getElementById("customMetalFormat")?.value || "word";
    if (!baseUrl || !endpoint) {
      alert("Please enter base URL and endpoint for custom provider");
      return;
    }
    config.custom = { baseUrl, endpoint, metalFormat };
    const display = document.getElementById("customBaseUrlDisplay");
    if (display) display.textContent = baseUrl;
  }
...
const showSettingsModal = () => {
  ...
  Object.keys(API_PROVIDERS).forEach((prov) => {
    ...
    if (prov === "CUSTOM") {
      ...
      const format = document.getElementById("customMetalFormat");
      const display = document.getElementById("customBaseUrlDisplay");
      if (baseUrl) baseUrl.value = currentConfig.custom?.baseUrl || "";
      if (endpoint) endpoint.value = currentConfig.custom?.endpoint || "";
      if (format) format.value = currentConfig.custom?.metalFormat || "word";
      if (display)
        display.textContent = currentConfig.custom?.baseUrl || "User-defined";
    }
  });
  ...
  if (body) {
    if (providerKey === "CUSTOM") {
      ...
      body.innerHTML = `
        ...
        <div>Base URL: ${custom.baseUrl || "Not set"}</div>
        <div>Endpoint: ${custom.endpoint || "Not set"}</div>
        <div>Metal parameter: ${
          custom.metalFormat === "symbol"
            ? "Symbol (XAG, XAU, XPT, XPD)"
            : "Word (silver, gold, etc.)"
        }</div>
        <div class="api-key-info">
          <div>📋 <strong>API Key Management:</strong></div>
          <ul>
            <li>Endpoint must return JSON containing a price field.</li>
            <li>Use {API_KEY} and {METAL} placeholders in endpoint.</li>
